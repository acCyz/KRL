...
class TransE:
    ...
    def train(self, epochs):
        ...
        sans = SANS(self.triple_list, len(self.entity), len(self.relation), 10, 3, 1000, "fbk")
        ...
        for epoch in range(epochs):
            ...
            for k in range(nbatches):
                ...
                for triple in Sbatch:
                    ...
                    corrupted_triple = sans.getitem(triple)
                    #if (tr iple, corrupted_triple) not in Tbatch:
...
if __name__=='__main__':
    file1 = "D:\\testcode\\SANS-master\\codes\\FB15k\\"
    ...
    """
    sans = SANS(triple_list, len(entity2id), len(relation2id), 10, 3, 1000,"fbk")
    i=0
    for s in triple_list:
        n = sans.getitem(s)
        #print("选中",np.random.choice(n,1)[0])
        print(i)
        i = i+1
    """
    ...
    true_headx = {}
    true_tailx = {}
    for head, tail, relation in triple_list:
        head, tail, relation = int(head), int(tail), int(relation)
        if (head, relation) not in true_tailx.keys():
            true_tailx[(head, relation)] = []
        true_tailx[(head, relation)].append(tail)
        if (tail, relation) not in true_headx.keys():
            true_headx[(tail, relation)] = []
        true_headx[(tail, relation)].append(head)

    for tail, relation in true_headx:
        true_headx[(tail, relation)] = np.array(list(set(true_headx[(tail, relation)])))
    for head, relation in true_tailx:
        true_tailx[(head, relation)] = np.array(list(set(true_tailx[(head, relation)])))

    sans = SANS(triple_list, true_headx,true_tailx,len(entity2id), len(relation2id), 10, 3, 1000, "fbk")

    t = ['14426', '1702', '1023']
    if t not in triple_list:
    ...
    for k in range(9):
        # Sbatch:list
        Sbatch = random.sample(triple_list, 2000)
        print("CUR k: ", k)
        for s in Sbatch:
            print("当前采样得到", s)
            if s in temp:
               ...
               temp.append(s)
            """
            head, tail, relation = s
            head, tail, relation = int(head), int(tail), int(relation)
            print(true_headx[(tail, relation)])
            print(true_tailx[(head, relation)])"""
            corrupted_triple = sans.getitem(s)
            """
            if s not in triple_list:
...
# python -u codes/run.py --do_test --init models/TransE_FB15k-237_uniform --test_batch_size 16
# streamlit run D:/testcode/KGtest/codes/run.py --do_train --do_test --data_path data/wn18rr --model TransE  -nsm uniform -n 256 -b 500 -d 100 -g 9.0 -a 1.0 -lr 0.0001 --max_steps 1000 -save models/TransE_wn18rr_kkk --test_batch_size 16
# python -u codes/run.py --do_train --do_test --data_path data/wn18rr --model TransE  -nsm sans -n 256 -b 500 -d 100 -g 9.0 -a 1.0 -lr 0.0001 --max_steps 50000 -save models/TransE_wn18rr_zbzb --test_batch_size 16 -khop 4 -nrw 2000
# python -u codes/run.py --do_train --do_test --data_path data/wn18rr --model TransE  -nsm nscaching -n 256 -b 500 -d 100 -g 2.0 -a 1.0 -lr 0.0001 --max_steps 1000 -save models/TransE_wn18rr_zbzb --test_batch_size 16 -khop 4 -nrw 2000